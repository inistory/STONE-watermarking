...
class WmProcessorRandomMessageModel(WmProcessorBase):
    ...
    def decode_with_input_ids(self, input_ids, messages=None, batch_size=16, disable_tqdm=False,
        ...
        if len(all_log_Ps) == 0:
            print(f"[⚠️ Warning] Skipped decoding: no valid token positions for decoding.")
            decoded_messages = [0 for _ in range(len(messages))]
            dummy_confidences = [(-1, -1, 0.0) for _ in range(len(messages))]
            if non_analyze:
                return decoded_messages, (None, dummy_confidences)
            else:
                return decoded_messages, (None, dummy_confidences)
